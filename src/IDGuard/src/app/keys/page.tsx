  ...
  return (
    <div className="container-page py-4 sm:py-8">
      ...
      {!isLoading && keys.length === 0 && (
        <div className="text-center py-12">
          <p className="text-text-secondary font-body">No keys found</p>
        </div>
      )}
      ...
      <div className="space-y-2 sm:space-y-3">
        {keys.map((key) => (
          <div
            key={key.keyId}
            className="card-compact bg-card border border-border-card rounded-lg p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-card"
          >
            ...
            <div className="flex flex-wrap items-center gap-2">
              {(key.startDate || key.endDate) && (
                <div className="text-xs text-text-muted sm:text-right mr-0 sm:mr-3 hidden sm:block font-body">
              ...
              <button
                ...
                className="px-2.5 py-1.5 sm:py-1 rounded bg-warning-soft text-warning text-xs hover:bg-yellow-100 border border-yellow-200 transition-colors font-body"
              >
                Freeze
              </button>
              <button
                onClick={() => handleKeyAction(key.keyId, "unfreeze")}
                className="px-2.5 py-1.5 sm:py-1 rounded bg-success-soft text-success text-xs hover:bg-green-100 border border-green-200 transition-colors font-body"
              >
                Unfreeze
              </button>
              <button
                onClick={() => handleKeyAction(key.keyId, "delete")}
                className="px-2.5 py-1.5 sm:py-1 rounded bg-accent text-white text-xs hover:bg-accent-hover transition-colors font-body"
              >
                Delete
              </button>
            </div>

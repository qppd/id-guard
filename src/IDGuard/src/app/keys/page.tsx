  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#183B6B] font-heading">eKeys</h1>
        ...
        <button
          ...
          className="px-3 py-1.5 rounded bg-[#183B6B] text-white text-sm hover:bg-[#2A5CA5] transition-colors font-body"
        >
      ...
      {showForm && (
        <form onSubmit={handleSendKey} className="bg-white border border-[#E5E7EB] rounded-lg p-4 mb-6 space-y-3 shadow-card">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <input
              ...
              className="px-3 py-2 rounded bg-white border border-[#E5E7EB] text-[#1F2937] text-sm placeholder-[#9CA3AF] focus:outline-none focus:border-[#3B82F6]"
              required
            />
            <input
              type="text"
      ...
      <div className="space-y-2">
        {keys.map((key) => (
          <div
            key={key.keyId}
            className="bg-white border border-[#E5E7EB] rounded-lg p-4 flex items-center justify-between shadow-card"
          >
            <div>
              <p className="text-[#183B6B] text-sm font-heading font-semibold">
            ...
            <div className="flex items-center gap-2">
              {(key.startDate || key.endDate) && (
                <div className="text-xs text-[#9CA3AF] text-right mr-3 hidden sm:block font-body">
              ...
              <button
                ...
                className="px-2 py-1 rounded bg-yellow-50 text-[#F59E0B] text-xs hover:bg-yellow-100 border border-yellow-200 transition-colors font-body"
              >
                Freeze
              </button>
              <button
                onClick={() => handleKeyAction(key.keyId, "unfreeze")}
                className="px-2 py-1 rounded bg-green-50 text-[#22C55E] text-xs hover:bg-green-100 border border-green-200 transition-colors font-body"
              >
                Unfreeze
              </button>
              <button
                onClick={() => handleKeyAction(key.keyId, "delete")}
                className="px-2 py-1 rounded bg-[#183B6B] text-white text-xs hover:bg-[#2A5CA5] transition-colors font-body"
              >
                Delete
              </button>
            </div>
